...
// Define ordered puroks 1-11 instead of querying from database
$puroks = [];
for ($i = 1; $i <= 11; $i++) {
    $puroks[] = "Purok " . $i;
}
...
<div class="col-md-6 text-end">
   ...
   <div class="container">
    ...
    <div class="tab-navigation">
        <a href="personnel_residence.php" class="nav-tab active">
       ...
       <a href="Personnel_request_history.php" class="nav-tab">
        ...
        <div class="page-header">
            ...
            <p class="page-subtitle">View all registered residents</p>
        </div>

        <!-- Stats Grid -->
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-number" id="totalResidents">
                    <?php
            ...
            <div class="stat-card">
                <div class="stat-number" id="maleCount">
                    <?php
                    $maleSql = "SELECT COUNT(*) as male FROM residents WHERE gender = 'Male'";
            ...
            <div class="stat-card">
                <div class="stat-number" id="femaleCount">
                    <?php
                    $femaleSql = "SELECT COUNT(*) as female FROM residents WHERE gender = 'Female'";
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">Active Puroks</div>
            </div>
        </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    <div class="position-relative">
                        <input type="text" id="searchInput" class="form-control search-input" placeholder="Search residents by name...">
                        <i class="fas fa-search search-icon"></i>
                    </div>
                </div>
                <div class="col-md-3">
                    <select id="purokFilter" class="form-select filter-select">
        ...
        <div class="data-table-container">
            <div class="table-responsive">
                <table class="table" id="residentsTable">
                    <thead>
                        <tr>
                            <th><i class="fas fa-user-circle me-2"></i>Profile</th>
                            <th><i class="fas fa-user me-2"></i>Full Name</th>
                            <th><i class="fas fa-venus-mars me-2"></i>Gender</th>
                            <th><i class="fas fa-birthday-cake me-2"></i>Birthdate</th>
                            <th><i class="fas fa-phone me-2"></i>Contact</th>
                            <th><i class="fas fa-map-marker-alt me-2"></i>Purok</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php
                    $sql = "SELECT * FROM residents ORDER BY last_name, first_name";
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $fullName = trim($row['first_name'] . ' ' . $row['middle_name'] . ' ' . $row['last_name']);
                            $genderClass = strtolower($row['gender']) === 'male' ? 'gender-male' : 'gender-female';
                            
                            echo "<tr data-purok='" . htmlspecialchars($row['purok']) . "' data-gender='" . htmlspecialchars($row['gender']) . "'>
                                <td>
                                    <img src='uploads/" . (!empty($row['profile_image']) ? htmlspecialchars($row['profile_image']) : 'default-profile.jpg') . "' 
                                         class='profile-img' alt='Profile'>
                                </td>
                                <td>
                                    <div class='resident-name'>" . htmlspecialchars($fullName) . "</div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class='purok-badge'>" . htmlspecialchars($row['purok']) . "</span>
                                </td>
                            </tr>";
                        }
                    } else {
                        echo "<tr><td colspan='6' class='empty-state'>
                                ...
                                <p>Start by adding some residents to the database.</p>
                              </td></tr>";
                    }
                    ?>
                    </tbody>
...
<?php endif; ?>
    ...
    <script>
        ...
        const table = document.getElementById('residentsTable').getElementsByTagName('tbody')[0];
        const rows = table.getElementsByTagName('tr');

        // Combined filter function with enhanced performance
        function filterTable() {
            const searchFilter = searchInput.value.toLowerCase().trim();
            const selectedPurok = purokFilter.value.toLowerCase();
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                const rowPurok = row.getAttribute('data-purok')?.toLowerCase() || '';
                ...
                const isVisible = matchesSearch && matchesPurok && matchesGender;
        ...
        function updateVisibleCount(count) {
            ...
            console.log(`Showing ${count} residents`);
        }
        ...
        document.querySelectorAll('.table tbody tr').forEach(row => {
            row.addEventListener('click', function() {
                ...
                console.log('Selected resident:', this.querySelector('.resident-name')?.textContent);
            });
        });
